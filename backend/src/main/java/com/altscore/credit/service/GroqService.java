...
public class GroqService {
    ...
    public String generateCombinedAnalysis(String businessName, String businessType,
            ...
            String riskLevel, int qualifiedLoans,
            int almostLoans) {
        String prompt = String.format("""
                ...
                - Credit Score: %.0f / 100
                - Risk Level: %s
                - Qualified for: %d KSA bank/program(s)
                - Almost qualifying for: %d more bank(s)

                Respond in this EXACT format:

                EXPLANATION:
                [2-3 sentences explaining the score, what it means,
                and specifically mention how many banks they qualify for]

                RECOMMENDATIONS:
                1. [Specific action relevant to KSA market] | Impact: +X to +Y points | Timeline: X months
                2. [Specific action relevant to KSA market] | Impact: +X to +Y points | Timeline: X months
                3. [Specific action relevant to KSA market] | Impact: +X to +Y points | Timeline: X months
                """,
                businessName, businessType, monthlyRevenue,
                yearsInOperation, numTransactions, creditScore,
                riskLevel, qualifiedLoans, almostLoans);
    ...
    private String generateFallbackAnalysis(String businessName, String businessType,
        ...
        if (creditScore >= 80) {
            explanation = businessName
                    + " demonstrates excellent creditworthiness with strong performance across all key metrics. " +
            ...
            recommendations = "1. Apply for a business expansion loan up to SAR 500,000 at favorable rates | Impact: Business growth | Timeline: 1-2 months\n"
                    +
            ...
            recommendations = "1. Increase monthly transactions from " + numTransactions
                    + " to above 100 by accepting STC Pay and Mada | Impact: +8 to +12 points | Timeline: 2-3 months\n"
                    +
                    "2. Maintain consistent revenue above SAR " + String.format("%.0f", monthlyRevenue * 1.2)
                    + " for 3 consecutive months | Impact: +5 to +8 points | Timeline: 3 months\n" +
            ...
            recommendations = "1. Increase monthly revenue to above SAR 20,000 through expanded sales channels | Impact: +10 to +15 points | Timeline: 3-6 months\n"
                    +
            ...
            recommendations = "1. Register business officially with Ministry of Commerce (CR number) | Impact: +15 points | Timeline: 2-4 weeks\n"
                    +
